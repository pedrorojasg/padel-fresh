...
const TYPES = [
  {
    ...
    description: 'Players team up once and every player will be collecting points individually.',
    available: true,
  },
  {
    id: 'mixed_americano',
    label: 'Mixed Americano',
    description: 'Mixed-gender teams are created, every player will be collecting points individually.',
    available: false,
  },
  {
    id: 'team_americano',
  ...
  {
    id: 'club_americano',
    label: 'Club Americano',
    description: 'Players from different clubs compete. Pairs are formed within the same club.',
    available: false,
  },
  {
    id: 'club_team_americano',
    label: 'Club Team Americano',
    description: 'Fixed teams from different clubs compete against each other.',
    available: false,
  },
];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black px-5">
      ...
      <div className="flex flex-col items-center mt-10 flex-1">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 text-center">Choose The Tournament Type</h1>
        <p className="text-muted text-sm mb-6 text-center uppercase tracking-widest">Americano</p>

        <div className="w-full bg-card rounded-2xl overflow-hidden border border-border/40">
          {TYPES.map((t) => (
            <button
              key={t.id}
              onClick={() => t.available && dispatch({ type: 'SET_TYPE', payload: t.id })}
            ...
            >
              <div>
                <div className={`font-semibold text-base ${t.available ? 'text-white' : 'text-muted'}`}>{t.label}</div>
